...
-- Date:              Sun Dec 10 13:08:46 2023 (by Create and Import Peripheral Wizard)
...
architecture IMP of user_logic is
  ...
  signal r3_count : std_logic_vector(31 downto 0);
  ------------------------------------------
...
begin
	...
	OPERATION_PROCESS: process ( slv_reg0, slv_reg1, slv_reg2, Bus2IP_Clk, Bus2IP_Reset ) is 
	begin
	...
	CLK_1HZ_PROCESS : process( Bus2IP_Clk, Bus2IP_Reset ) is
    begin
      if (Bus2IP_Reset = '1') then
        clk_1hz <= '0';
        count <= (others => '0');
      elsif rising_edge(Bus2IP_Clk) then
        if count = "00000001011111010111100001000000" then
          count <= (others => '0');
          clk_1hz <= not clk_1hz;
        else
          count <= count + 1;
	...
	COUNT_PROCESS : process( clk_1hz, Bus2IP_Reset ) is
    begin
      if (Bus2IP_Reset = '1') then
        r3_count <= (others => '0');
      elsif rising_edge(clk_1hz) then
        if r3_count = slv_reg3 then
          r3_count <= (others => '0');
        else
          r3_count <= r3_count + 1;
        end if;
...
leds <=
	r3_count(7 downto 0) when switches(7) = '1' else
